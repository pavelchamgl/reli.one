...
**Важно:** сценарий относится к **локальному e2e** (`docker-compose.e2e.yml`, sandbox credentials, Mailpit). Это **не** production-приёмка.

---

## Verification evidence (заполнить после первого успешного прогона)

Скопируйте таблицу и добавьте строки фактов (ids, номера инвойсов) **только локально** или в тикет — не в публичный репозиторий с PII.
...
| PayPal `order_id` / `session_key` | замаскировать при публикации |
| Webhook доставка | HTTP от backend; код в PayPal Dashboard (если доступен) |
| Payment / Orders / Invoice | id / номер (опционально) |

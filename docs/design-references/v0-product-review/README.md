...
## Локальные скриншоты (`public/`)

Основные reference-кадры для Iteration 7.6:

| Файл | Назначение |
|------|------------|
| `product-main.png` | Desktop product review |
| `review-final.png` | Итоговая композиция |
| `review-sticky.png` | Sticky gallery |
| `review-logo-fix.png` | Breadcrumb / logo alignment |

Дополнительные пользовательские скриншоты (`User attachment*.png`) — вспомогательные, не обязательны для CI; коммитить только при явной необходимости в задаче.
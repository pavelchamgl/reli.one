...
### DEV-4: Строка `.gitignore` для `*/migrations` `P2` — **снято в репозитории (2026-05)**

Паттерн `*/migrations` **удалён** из [`.gitignore`](../.gitignore); новые файлы миграций Django отслеживаются как обычные. Строка `*/migrations/__pycache__` сохранена.

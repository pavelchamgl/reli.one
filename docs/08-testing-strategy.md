...
| Frontend | **`Frontend3`:** Vitest + RTL; `npm run test:e2e` (Playwright); см. [`docs/frontend/README.md`](frontend/README.md), [`test-matrix`](frontend/test-matrix.md). **`Frontend2`:** Vitest (`landing-smoke.test.js`); ESLint **0 errors** (warnings допустимы); job **`frontend2`** — lint → test → build. План: **[`testing-plan`](frontend/testing-plan.md)**. |
...
| CI | `.github/workflows/ci.yml`: backend — migrations, `manage.py test`, `pytest`; **`frontend2`** — `npm ci`, lint, `npm run test`, build; **`frontend3`** — `npm ci`, lint, `npm run test`, build; **`e2e_frontend3`** — build, Playwright Chromium, `npm run test:e2e`. Детали: [`docs/frontend/README.md`](frontend/README.md). |
...
- **Frontend:** `Frontend3` — `npm run test` в CI; `Frontend2` — `npm run test` + lint + build в CI.

...
- Учтено **фактическое** состояние: backend использует `django.test` / `APITestCase` / `APIClient` и **pytest-django** (тот же тестовый набор можно гнать и через `manage.py test`, и через `pytest`); осмысленные тесты есть в `accounts`, `sellers`, `payment`, `order`, `product`, `delivery`, `promocode`, складские регрессии в **`warehouses/tests_stock.py`** (см. **Task 009**; `warehouses/tests.py` может оставаться пустым); `backend/pytest.ini` и `backend/conftest.py` есть; **`Frontend3`**: `npm run test` (Vitest + RTL), `npm run test:e2e` (Playwright smoke), см. [`docs/frontend/testing-plan.md`](frontend/testing-plan.md); **`Frontend2`**: `npm run test` (Vitest smoke). Матрица: [`docs/frontend/test-matrix.md`](frontend/test-matrix.md).
...
| Frontend | **`Frontend3`:** Vitest + Testing Library; `npm run test:e2e` (Playwright); `renderWithProviders`; P0-регрессии API и `ProtectedRoute`; CI — `frontend3` + **`e2e_frontend3`**. **`Frontend2`:** Vitest smoke, CI — шаг `npm run test`. План и матрица: **[testing-plan](frontend/testing-plan.md)**, **[test-matrix](frontend/test-matrix.md)**, **[tasks](frontend/tasks/README.md)**. |
...
| CI | `.github/workflows/ci.yml`: backend (migrations + `manage.py test` + `pytest`); **`frontend2`**: `npm ci`, lint, **`npm run test`**, build; **`frontend3`**: `npm ci`, lint, **`npm run test`**, build; **`e2e_frontend3`**: build + Playwright Chromium + **`npm run test:e2e`**. |

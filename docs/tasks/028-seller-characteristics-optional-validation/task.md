...
- [x] На `/seller/seller-create`: сценарий «Add item → ввод → удаление строки»
...
- [x] На `/seller/seller-edit/:id`: тот же сценарий не блокирует сохранение.
- [x] Частично заполненная строка (заполнено только `name` **или** только
      `value`) по-прежнему даёт ошибку `allParametersAreRequired`.
- [x] Полностью заполненные строки сохраняются и проходят валидацию как раньше
      (регресс не внесён).
- [x] Логика «пустой строки» вынесена в общий хелпер и используется обеими
      формами (нет копипасты условия).
- [x] Добавлены тесты: unit на хелпер + сценарные тесты add/delete для create и
      edit; существующие тесты зелёные.
- [x] `npm run lint` и `npm run test` во Frontend3 зелёные.
- [x] Документация обновлена: этот `task.md` отмечен, поведение описано.
...
### Статус
- [x] Analysis complete — первопричина подтверждена: `handleDelete` пишет в Redux
      `[{ name: "", value: "" }]`, а `isParametersValid` через `every(...)` требует
      заполнения. Дублирование в create (`SellerCreateForm`) и edit
      (`EditGoodsForm`) подтверждено.
...
### Статус
- [x] Tests written (red) — добавлены unit-тесты `isEmptyParameterRow` /
      `areProductParametersValid` и сценарные тесты для `CreateCharacInp` и
      `EditGoodsParameters`. До правки падали (`areProductParametersValid is not
      a function`).
...
### Статус
- [x] Fix done — в `sellerProductParameters.js` добавлены `isEmptyParameterRow`
      и `areProductParametersValid` (переиспользуют `getVisibleProductParameters`).
      Обе формы используют `areProductParametersValid(...)` вместо инлайн-условия.
      Сериализация на отправке и логика атрибутов категории не тронуты.
...
- [x] create: add → ввод → delete → «Предпросмотр» проходит (покрыто тестом
      `CreateCharacInp` «stays optional after add -> type -> delete»).
- [x] create: add → только `name` → ошибка `allParametersAreRequired`
      (тест «becomes invalid when a row has only the name filled»).
- [x] create: две полностью заполненные строки → проходит (unit-тест
      `areProductParametersValid` «treats fully filled rows as valid»).
- [x] edit: удаление единственной строки → сохранение проходит (тест
      `EditGoodsParameters` «stays optional after deleting the only filled row»).
- [x] edit: частично заполненная строка → ошибка (тест `EditGoodsParameters`
      «becomes invalid when a row has only the name filled»).
- [x] Блок, который не трогали, по-прежнему опционален (unit-тест «treats
      untouched / empty input as valid»).
- [x] `npm run lint` ✅ (0 errors), `npm run test` ✅ (55 файлов, 474 теста).
...
- `Frontend/Frontend3/src/Components/Seller/shared/sellerProductParameters.js`
  — добавлены `isEmptyParameterRow` и `areProductParametersValid`.
- `Frontend/Frontend3/src/Components/Seller/create/sellerCreateForm/SellerCreateForm.jsx`
  — `isParametersValid = areProductParametersValid(product_parameters)`.
- `Frontend/Frontend3/src/Components/Seller/edit/EditGoodsForm/EditGoodsForm.jsx`
  — `isParametersValid = areProductParametersValid(parameters)`.

**Изменённые/новые файлы (тесты):**
- `Frontend/Frontend3/src/Components/Seller/shared/sellerProductParameters.test.js`
  — unit-тесты `isEmptyParameterRow` / `areProductParametersValid`.
- `Frontend/Frontend3/src/ui/Seller/create/createCharacteristicsInp/CreateCharacInp.test.jsx`
  — сценарии add/type/delete и partial-row.
- `Frontend/Frontend3/src/Components/Seller/edit/editGoodsParameters/EditGoodsParameters.test.jsx`
  — новый файл, сценарии delete-last-row и partial-row.

**Проверки (вне sandbox, окружение jsdom):**
- Целевые тесты: 19 passed (3 файла).
- Полный набор Frontend3: 55 files passed, 474 tests passed.
- `npm run lint`: 0 errors (654 пред-существующих warning, не в изменённых файлах).

**Поведение (без изменения контрактов и текстов):** блок «Characteristics»
снова опционален при любом сценарии; полностью пустые строки игнорируются;
частично заполненная строка остаётся ошибкой `allParametersAreRequired`;
backend-сериализация и атрибуты категории не затронуты.

...
- [ ] Страница деталей заказа показывает заказ текущего пользователя, не заказ #16
- [ ] Поиск товаров работает и возвращает JSON
- [ ] Страница онбординга продавца загружает правильный статус
- [ ] Список «текущих заказов» не пустой из-за пробела в query
- [ ] `BaseURL` читается из `VITE_API_URL` env variable
- [ ] Неавторизованный пользователь на `/seller/*` редиректится на login
- [ ] Авторизация работает без перезагрузки страницы

...
Legacy `LicenseFile` write endpoint принимает только PDF и DOCX по MIME:

- `application/pdf`;
- `application/vnd.openxmlformats-officedocument.wordprocessingml.document`.

Frontend validation теперь срабатывает сразу после выбора файла:

- PDF/DOCX разрешены;
- JPEG/JPG/PNG/DOC и другие форматы блокируются;
...
7. В create-flow выбрать invalid license file (`.jpg`, `.png`, `.doc`) и убедиться, что ошибка показана сразу.
8. В edit-flow повторить invalid license file check.
9. В edit-flow удалить существующий legacy license, затем загрузить валидный PDF/DOCX, если текущий товар это позволяет.

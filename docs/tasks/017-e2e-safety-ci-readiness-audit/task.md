...
**Status:** **DONE (repo-scope documentation verification)** — production-risk при корректном деплое низкий; full-stack e2e **local-only** до отдельной CI-задачи; runtime-фиксы не потребовались.

---

## Goal

Проверить безопасность test-only механизмов, появившихся после FS-003 (`ENABLE_E2E_ENDPOINTS`, `STRIPE_WEBHOOK_SKIP_SIGNATURE`, `/api/e2e/payment/*`), и определить, готов ли full-stack Playwright + Django e2e контур к интеграции в CI.

Результат — явный verdict: **local-only** vs **CI-ready**, матрица safety-проверок и proposal будущего CI job без его реализации.
...
Текущий CI job `e2e_frontend3` (`.github/workflows/ci.yml`) запускает только Playwright против Vite preview **без** Django/Postgres. Full-stack спеки (`fullstack-*.spec.js`) при этом **auto-skip**, если backend недоступен.
...
- CI readiness checklist и proposal job (без изменения workflow)
...
- Реализация CI job / изменение `.github/workflows/ci.yml`
...
| C-05 | Отдельный CI job для full-stack | **MISSING** | Текущий job не поднимает backend |
| C-06 | Env files для CI | **PARTIAL** | Нужны `envs/database.e2e.env` + `envs/backend.e2e.env` из examples (secrets в CI vars не нужны — пустые PSP) |
...
| C-09 | FS-001 в CI | **READY (proposal)** | Не требует e2e flags |
| C-10 | FS-002 в CI | **READY (proposal)** | Требует backend; Stripe 500 на пустом key — ожидаемо в FS-002a |
| C-11 | FS-003 в CI | **READY (proposal)** | Требует `ENABLE_E2E_ENDPOINTS` + `STRIPE_WEBHOOK_SKIP_SIGNATURE` в backend env |
| C-12 | Stripe/PayPal sandbox | **MANUAL** | Вне CI — чеклисты `docs/testing/stripe-e2e-checklist.md`, `paypal-e2e-checklist.md` |

### Verdict: local-only (сейчас)

Full-stack e2e **следует оставить local-only** до отдельной задачи на CI:

1. Текущий `e2e_frontend3` намеренно лёгкий (smoke + mocked API) и **не должен** ломаться из-за отсутствия Docker backend.
2. Full-stack job потребует 5–15+ минут (build backend image, migrate, build frontend, playwright), отдельные порты и e2e-only flags на runner.
3. FS-003 зависит от test-only endpoints — их **нельзя** включать на shared staging без изоляции.

Это согласуется с Task 015: Tier 3 (EXT-001/002) остаётся ручным sandbox.
...
### CI (P2, отдельная задача — **не Task 017**)

| ID | Описание |
|----|----------|
| FU-017-CI-01 | Новый workflow job `e2e_fullstack` (optional / nightly / label-triggered) |
| FU-017-CI-02 | Шаги: `docker compose -f docker-compose.e2e.yml up -d` → wait health → `npm ci && build` → `playwright test e2e/fullstack-*.spec.js` |
| FU-017-CI-03 | Env: скопировать `*.e2e.env.example` → `*.e2e.env` в CI без секретов |
| FU-017-CI-04 | Убедиться, что `backend/order/fonts/*.ttf` доступны в образе (FS-003 invoice PDF) |
| FU-017-CI-05 | Согласовать: FS-001+002+003 в одном job или FS-001 отдельно как быстрый gate |

### Proposal: будущий CI job (не внедрять в Task 017)
...
**Тесты в будущем CI job:** FS-001, FS-002, FS-003.  
**Вне CI (ручные):** Stripe sandbox, PayPal sandbox, external provider E2E (Task 015 Tier 3).
...
| Task | Связь |
|------|-------|
| **015** | Full-stack design; FS-001–003 реализованы; CI proposal здесь уточняет Tier 1 |
...
- [x] CI readiness checklist заполнен
- [x] Verdict: **local-only** для full-stack; CI — proposal only

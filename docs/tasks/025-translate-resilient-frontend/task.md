...
- **Phase 2 (возврат автоперевода RU/UK без краша):** Done (код + тесты;
  ручная проверка EN→RU/UK в Chrome — за продактом).
...
- `ErrorBoundary.test.jsx` — рендер детей; фолбэк + `Sentry.captureException`
  с тегами `area`, `translation_likely_active` (true при `<font>` в `#root` /
  false без него), `react.componentStack`; `translate="no"` на фолбэке.
- `main.bootstrap.test.js` — `installDomTranslateGuard()` до `createRoot`,
  `ErrorBoundary area="app"` оборачивает `RouterProvider`.
...
- [x] Перевод включён → заполнить форму невалидно → «Предпросмотр» → ошибки
      валидации показываются, краша нет.
- [x] Перевод включён → валидная форма → превью → «Отправить на модерацию» →
      успех, без белого экрана.
- [x] Принудительно вызвать ошибку рендера → виден наш фолбэк, событие в Sentry
      (`ErrorBoundary.test.jsx`: фолбэк + `captureException` + `componentStack`;
      `translation_likely_active` true/false; `main.bootstrap.test.js`: wiring).
      Доставка в Sentry UI — ops-only при боевом DSN.
- [x] Перевод выключен → поведение идентично прежнему (регресс не внесён).
- [x] `npm run lint` ✅, `npm run test` ✅ (Frontend3).
...
**Priority:** P1 · **Complexity:** Medium · **Status:** Done
...
- [x] Автоперевод браузера на RU и UK **включается** на `/seller/seller-create`
      и `/seller/seller-preview` (сняты `notranslate`/`translate="no"` с `#root`).
- [x] При включённом переводе заполнение формы, появление ошибок валидации,
      «Предпросмотр» и «Отправить на модерацию» **не должны вызывать** краш
      (DOM-guard + always-mounted `hidden`; подтверждено unit-тестами reparent).
- [x] `installDomTranslateGuard()` подключён в `main.jsx` до первого рендера,
      идемпотентен, «глотает» только cross-parent операции.
- [x] Технические/числовые токены защищены `translate="no"` (цена, sale price,
      stock, SKU, ID, VAT, габариты, warranty в превью/вариантах/create inputs).
- [x] i18n-тексты ошибок валидации снова переводимы (`translate="no"` снят),
      always-mounted `hidden`-паттерн сохранён.
- [x] Тесты: `domTranslateGuard.test.js`, обновлены Phase 1 тесты, новые тесты
      на техн. токены (`SellerReviewProductInfo`, `SellerCreateVariant`),
      расширен `ErrorBoundary.test.jsx`, `main.bootstrap.test.js` (wiring +
      Sentry tags без ручного `__crash`).
- [x] `npm run lint` (0 errors) и `npm run test` (485 passed) зелёные.
- [x] Ручная проверка: создание товара и редактирование с отправкой на модерацию
      прошли без ошибок (evidence: ручная проверка Pavel, июнь 2026).
...
`src/utils/domTranslateGuard.js` — `installDomTranslateGuard()` патчит
`Node.prototype.removeChild`/`insertBefore` (cross-parent no-throw, идемпотентно).
Подключён в `main.jsx` до `ReactDOM.createRoot`.
...
Сняты `notranslate` с `index.html` и `#root`. С `ошибок валидации` снят
`translate="no"`. На техн. токены добавлен `translate="no"`: цены/SKU/VAT в
`SellerReviewProductInfo`, габариты/warranty в `SellerReviewDetailsSections`,
числовые inputs в `SellerCreateVariant` и `CreateFormInp` (num/digitsOnly/decimal),
Product ID и step-коды в баннере `partial_success`.
...
`domTranslateGuard.test.js` (5), обновлены CreateFormInp/CreateCharacInp/
SellerCreateImage/SellerPreviewPage тесты, новые SellerReviewProductInfo.test.jsx
и SellerCreateVariant.test.jsx; регрессии в sellerProductWizardSlices.test.js
обновлены под split SKU/VAT markup. Дополнительно: `ErrorBoundary.test.jsx`
(translation_likely_active true/false, componentStack), `main.bootstrap.test.js`.
...
- `npm run test` → 59 файлов, 485 тестов passed (вкл. ErrorBoundary + bootstrap wiring).
- Ручная проверка: создание товара + редактирование + отправка на модерацию
  без ошибок (Pavel, июнь 2026).
...
| **Tests** | новые `*.test.jsx`/`*.test.js` рядом с компонентами, `src/utils/domTranslateGuard.test.js`, `src/main.bootstrap.test.js` |

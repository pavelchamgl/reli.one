...
## Рекомендуемая приоритизация (roadmap)

1. Завершить **структурные** шаги **Task 004** (Order domain backlog), где это уменьшает хрупкость статусов и FK — **до** включения списания со склада в webhook.
2. **Не** вызывать `decrease_stock` из webhook, пока нет резерва/TTL на этапе create payment session — см. раздел *Риск* и *Явное правило* ниже.
3. Согласовать с **Task 003** точки входа checkout, когда появится дизайн резерва.

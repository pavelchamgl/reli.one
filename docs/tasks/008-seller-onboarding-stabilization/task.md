...
3. **Финальная валидация:** прогон `pytest backend/sellers/ -v` / `manage.py test sellers` + ручной чеклист из Iteration 5 перед merge крупных refactor-веток.
...
- [ ] Регрессии по шагам и странам на уровне, достаточном для безопасного рефакторинга: **`compute_completeness`** и связанные функции — `test_onboarding_completeness.py`; **`OnboardingAuditLog`** для сервисных переходов и `log_onboarding_event` — `test_onboarding_audit.py`; при необходимости полные REST-цепочки по странам.
...
Историческая ссылка на `tests_onboarding_flow.py` **не соответствует** репозиторию: расширять **`backend/sellers/test_onboarding_stabilization.py`** и/или **`backend/sellers/tests.py`** / **`backend/sellers/test_onboarding_completeness.py`** / **`backend/sellers/test_onboarding_audit.py`**.
...
- [ ] REST/API цепочки по странам (если нужны продукту) — открыто
...
| **Тесты** | `sellers/tests.py`, `sellers/test_onboarding_stabilization.py`, `sellers/test_onboarding_completeness.py`, `sellers/test_onboarding_audit.py` |

...
1. **`compute_completeness`:** отдельные **unit/integration** тесты на ветки (тип продавца, недостающие документы, граничные случаи документов front/back, `is_submittable`) — сейчас логика затрагивается опосредованно через ответ state, **без** целенаправленного покрытия веток. *(Числового процента прогресса в API нет — см. flow-документ.)*
2. **Company / Self-employed по странам:** API-regression по полным цепочкам для **CZ/SK** и хотя бы один контрольный кейс для другой страны ЕС **на уровне полей**, если продукт это требует (в `compute_completeness` отдельной матрицы стран **нет** — см. [`seller-onboarding-flow.md`](../seller-onboarding-flow.md)).
3. **Audit log:** тесты, проверяющие создание **`OnboardingAuditLog`** (не только `@patch("sellers.services_onboarding.log_onboarding_event")` в transition-тестах).
4. **Декомпозиция `views_onboarding.py`** в step-handlers — **только после** расширения регрессий (как в Iteration 3 ниже); URL и контракты не менять.
5. **Финальная валидация:** прогон `pytest backend/sellers/ -v` / `manage.py test sellers` + ручной чеклист из Iteration 5 перед merge крупных refactor-веток.

...
1. **Company / Self-employed по странам:** API-regression по полным цепочкам для **CZ/SK** и хотя бы один контрольный кейс для другой страны ЕС **на уровне полей**, если продукт это требует (в `compute_completeness` отдельной матрицы стран **нет** — см. [`seller-onboarding-flow.md`](../seller-onboarding-flow.md); инвариант зафиксирован в `test_onboarding_completeness.py`).
2. **Audit log:** тесты, проверяющие создание **`OnboardingAuditLog`** (не только `@patch("sellers.services_onboarding.log_onboarding_event")` в transition-тестах).
3. **Декомпозиция `views_onboarding.py`** в step-handlers — **только после** расширения регрессий (как в Iteration 3 ниже); URL и контракты не менять.
4. **Финальная валидация:** прогон `pytest backend/sellers/ -v` / `manage.py test sellers` + ручной чеклист из Iteration 5 перед merge крупных refactor-веток.
...
- [ ] Регрессии по шагам и странам на уровне, достаточном для безопасного рефакторинга: **`compute_completeness` и связанные сервисные функции** — есть **`test_onboarding_completeness.py`**; остаётся **audit** (`OnboardingAuditLog`) и при необходимости полные REST-цепочки по странам.
...
Историческая ссылка на `tests_onboarding_flow.py` **не соответствует** репозиторию: расширять **`backend/sellers/test_onboarding_stabilization.py`** и/или **`backend/sellers/tests.py`** / **`backend/sellers/test_onboarding_completeness.py`**.

- ~~Явные кейсы для **`compute_completeness`**~~ — см. **`test_onboarding_completeness.py`** (шаг 3).
- Цепочки **company / self-employed** по REST (полные happy-path) при продуктовой необходимости.
- Утверждения по **`OnboardingAuditLog`** (реальные записи в БД).
...
- [ ] REST/API цепочки по странам (если нужны продукту), audit asserts — открыто
...
| **Тесты** | `sellers/tests.py`, `sellers/test_onboarding_stabilization.py`, `sellers/test_onboarding_completeness.py` |

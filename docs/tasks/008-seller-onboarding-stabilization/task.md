...
- **Декомпозиция views:** пакет `sellers/onboarding/` — **шаг 6:** state / review / submit; **шаг 7:** `SellerSetSellerTypeAPIView` → `steps/seller_type.py`; **шаг 8:** self-employed personal / tax / address → `steps/self_employed.py`; реэкспорт из `views_onboarding.py` для `urls.py`.
...
- [ ] Остальные step-handlers (company-блоки, bank, warehouse, return, documents) — перенос малыми шагами
...
├── __init__.py
├── steps/
│   ├── __init__.py      # экспорт seller_type + self-employed views
...
| **Onboarding views (пакет)** | `sellers/onboarding/steps/state.py`, `sellers/onboarding/steps/seller_type.py`, `sellers/onboarding/steps/self_employed.py`, `sellers/onboarding/review/review.py`, `sellers/onboarding/review/submit.py` (реэкспорт в `views_onboarding.py`) |

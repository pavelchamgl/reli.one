...
> **Навигация по номерам:** **[Task 004](../004-order-consistency/task.md)** — Order Consistency + **фиксация Payment cleanup repo-scope** ([Final DoD](../004-order-consistency/task.md#final-dod-table)). Текущий документ — **payment flow / cleanup** в коде и доках (канон постановки).

**Status:** **DONE (репозиторий)** — P0-ядро платежей и **payment cleanup repo-scope** (май 2026): финальный аудит и таблица DoD — **[Task 004](../004-order-consistency/task.md#final-dod-table)** (им же зафиксирован regression gate). **Production / live PSP acceptance** git **не** утверждает. **OPEN — необязательный polish:** структура `payment/services/*`, расширенное логирование, доп. негативные ветки webhook — см. **Open** ниже; **не блокируют** закрытие repo-scope. Промокоды — **Deferred**. **Task 013** — implementation **Done (repo-scope)**; rollout **ops open**; **не** входит в scope **003** (см. ниже).
...
Устранить P0-риски надёжности payment flow (дублирование заказов по платежу, гонки инвойсов PAY-4) и выделить сервисный слой для безопасной поддержки. **Промокоды** в рамках текущего roadmap **не** являются целью этой задачи. **Stock reservation** реализован в **Task 013** (repo-scope); rollout — ops; **не** входит в scope **003** — см. **Deferred**.
...
**Явно не входит в scope Task 003 (текущий roadmap):** промокоды в продукте; включение **Task 013** rollout на staging/prod (реализация reservation/decrease_stock уже в **013**, см. **Deferred**).
...
### Open (optional polish — не блокирует repo-scope closure)

Закрытие **payment cleanup** по репозиторию зафиксировано в **[Task 004](../004-order-consistency/task.md)** (таблица DoD + regression). Ниже — хвосты для улучшения без смены контрактов:
...
- [ ] **Финальный audit:** периодическая сверка этого `task.md` с кодом при крупных изменениях `payment/`
...
| **Stock reservation / `decrease_stock` / warehouse quantity** | Реализовано в **Task 013** (repo-scope); rollout **ops open**; **не** входит в обязательный scope **003** |
...
sequenceDiagram
    ...
    Note over Backend,DB: Warehouse / reservation + decrease_stock — Task 013 (repo done; rollout ops open), не в scope 003
...
- **Списание склада / reservation** — реализовано в **Task 013** (repo-scope); rollout **ops open** (**не** хвост **003**)

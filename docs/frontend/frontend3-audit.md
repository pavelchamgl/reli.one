...
#### FE-P3-001: ✅ Checkout happy path e2e — Done (FE-009)

**Severity:** P3 ✅ Fixed  
**Описание:**  
Playwright smoke (FE-008) проверял routing и mounting. FE-009 добавил 6 e2e тестов:  
basket (empty/seeded) + basket→payment navigation + payment section 1 + no PSP calls.  
Полный сценарий sections 2–3 (delivery + payment form) — отдельный follow-up.  
**Файл:** `e2e/checkout.spec.js`  
**Follow-up:** FE Phase 4 (PR 4.2) — checkout sections 2–3.

---

#### FE-P3-002: ✅ Seller onboarding e2e smoke — Done (FE-010)

**Severity:** P3 ✅ Fixed  
**Описание:**  
FE-010 добавил 4 e2e smoke теста: login page, create-account page, seller-type page (мок GET /sellers/onboarding/state/), application-sub page (мок state pending_verification).  
Полный onboarding flow (SellerInformation → Review → Submit) — отдельный follow-up.  
**Файл:** `e2e/seller-onboarding.spec.js`
...
| Checkout e2e happy path | ✅ 6 тестов: basket/payment section 1/no PSP calls (FE-009) — `e2e/checkout.spec.js` |
| Seller onboarding e2e smoke | ✅ 4 теста: login/create-account/seller-type/application-sub (FE-010) — `e2e/seller-onboarding.spec.js` |
| Login/reg форм | **Backlog** — не реализованы |
| API errors/retry | **Backlog** — не реализованы |
| Basket/checkout RTL | **Backlog** — не реализованы |
| Checkout e2e sections 2–3 | **Backlog** — delivery mock + payment form (follow-up FE-009) |
| Seller onboarding full flow e2e | **Backlog** — SellerInformation → Review → Submit (follow-up FE-010) |
...
| FE-P3-001 | P3 ✅ Fixed | E2E | Checkout e2e happy path — 6 тестов (FE-009), `e2e/checkout.spec.js` |
| FE-P3-002 | P3 ✅ Fixed | E2E | Seller onboarding e2e smoke — 4 тестов (FE-010), `e2e/seller-onboarding.spec.js` |

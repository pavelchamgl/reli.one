...
| `review_requested` | Успешный `submit_application`. |
| `moderation_approved` / `moderation_rejected` | Admin approve/reject. |
...
| Views | `backend/sellers/views_onboarding.py` |
...
|------|-------------------------|
| `backend/sellers/tests.py` | Держатель счёта компании, OSVČ personal API; сервисные `submit_application` / `approve_application` / `reject_application`. |

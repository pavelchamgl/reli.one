...
- Для self-employed onboarding Task 023 MVP добавляет только frontend ARES assist/autofill: preview по Czech IČO, явный Apply в пустые business/tax/address поля и ручное редактирование. Submit-time ARES verification для self-employed не выполняется.
...
Для company onboarding между строгой валидацией и переходом в `pending_verification` выполняется ARES verification hint:

- lookup по `SellerCompanyInfo.business_id` / IČO;
- сравнение `business_id`, `company_name`, `legal_form`, registered company address при достаточно полной normalized address data, и активности компании;
- сохранение только sanitized snapshot (`SellerAresVerification.normalized`), `is_active`, `field_matches`, `checked_at`;
- audit `ares_verified` при совпадении проверенных полей и активной компании, иначе `ares_mismatch`;
...
Self-employed ARES assist в Task 023 MVP не меняет submit path: registry name остаётся preview-only, Apply не заполняет `first_name` / `last_name`, phone, bank, warehouse, return address или документы, а заявка после submit также ждёт ручной модерации.
...
Для company-заявок после submit админка показывает read-only ARES moderator hint panel:

- queried IČO;
- checked timestamp;
- active/inactive status;
- normalized ARES company name, legal form and registered address;
- field match/mismatch summary;
- warnings, including inactive company or address partial/mismatch.
...
| `ares_lookup` | Seller вызвал company ARES lookup endpoint. Payload хранит sanitized outcome (`found`, `not_found`, `invalid`, `unavailable`) и IČO без raw response. |
| `ares_verified` | Submit-time ARES verification для company успешно нашёл активную компанию и проверенные поля совпали. |
| `ares_mismatch` | Submit-time ARES verification нашёл компанию, но есть расхождения в checked fields или компания неактивна. |

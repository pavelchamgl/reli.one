...
- Для company onboarding после `validate_before_submit` backend делает non-blocking ARES-сверку по `company_info.business_id` как **moderator hint**. Результат сохраняется в `SellerAresVerification` как sanitized snapshot и audit-событие `ares_verified` или `ares_mismatch`. ARES not found/unavailable/error **не блокирует submit**.
- Для self-employed onboarding Task 023 MVP добавляет frontend ARES assist/autofill: preview по Czech IČO, явный Apply в пустые business/tax/address поля и ручное редактирование. При submit backend также делает non-blocking ARES-сверку по `self_employed_tax.business_id` как **moderator hint**.
...
Для company и self-employed onboarding между строгой валидацией и переходом в `pending_verification` выполняется ARES verification hint, если в соответствующем блоке есть IČO/business ID:

- company: lookup по `SellerCompanyInfo.business_id` / IČO; сравнение `business_id`, `company_name`, `legal_form`, registered company address при достаточно полной normalized address data, и активности компании;
- self-employed: lookup по `SellerSelfEmployedTaxInfo.business_id` / IČO; сравнение `business_id`, registered/primary self-employed address при достаточно полной normalized address data, и активности записи в реестре;
- self-employed registry/person name может показываться как moderator hint, но не используется как false mismatch, если соответствие с профилем продавца ненадёжно;
- сохранение только sanitized snapshot (`SellerAresVerification.normalized`), `is_active`, `field_matches`, `checked_at`;
- audit `ares_verified` при совпадении проверенных полей без warnings, иначе `ares_mismatch`;
- ARES failures (`not_found`, unavailable, unexpected error) не добавляют auto-decision и не блокируют submit.

Auto-approve отсутствует: даже при полном совпадении заявка остаётся `pending_verification` и ждёт ручной модерации.

Self-employed ARES assist в Task 023 MVP остаётся public-registry helper: registry name остаётся preview-only, Apply не заполняет `first_name` / `last_name`, `date_of_birth`, `nationality`, phone, bank, warehouse, return address или документы, а заявка после submit также ждёт ручной модерации.
...
Для company и self-employed заявок после submit админка показывает read-only ARES moderator hint panel, если submit-time lookup был выполнен:

- queried IČO;
- checked timestamp;
- active/inactive status;
- normalized ARES company/registry name, legal form/registry type where available, and registered address;
- field match/mismatch summary;
- warnings, including inactive registry status, address partial/mismatch, unavailable/not found, or self-employed fields that were intentionally not checked.

Полный raw response ARES в админке и БД не хранится.
...
| `ares_lookup` | Seller вызвал ARES lookup endpoint. Payload хранит sanitized outcome (`found`, `not_found`, `invalid`, `unavailable`) и IČO без raw response. В Task 023 self-employed frontend assist переиспользует company-named lookup endpoint. |
| `ares_verified` | Submit-time ARES verification для company или self-employed успешно нашёл активную запись и проверенные поля совпали без warnings. |
| `ares_mismatch` | Submit-time ARES verification нашёл запись с расхождениями/warnings, неактивную запись, либо ARES был unavailable/not found/error; submit не блокируется. |
...
| GET | `onboarding/company/ares-lookup/` | `seller-onboarding-company-ares-lookup` | Czech-only stateless ARES lookup по IČO для company prefill и Task 023 self-employed frontend assist; данные не сохраняет. |

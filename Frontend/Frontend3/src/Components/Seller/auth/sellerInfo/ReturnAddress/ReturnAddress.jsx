...
const ReturnAddress = ({ formik }) => {
    ...
    const onLeaveReturnBlock = async () => {
        ...
        try {
            const proofDocumentIssueDate = toISODate(formik.values.rProof_document_issue_date)

            await putReturnAddress({
                ...payload,
                ...(proofDocumentIssueDate ? { proof_document_issue_date: proofDocumentIssueDate } : {})
            })
        } catch (err) {

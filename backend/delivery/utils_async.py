...
def run_parcels_and_seller_email_after_commit(order_ids, session_id: str) -> None:
    """
    Синхронное выполнение цепочки parcel → labels → seller → manager.
    Вызывается из фонового потока после commit; вынесено для юнит-тестов без executor.
    """
...
def async_parcels_and_seller_email(order_ids, session_id: str):
    ...
    """
    def _target():
        run_parcels_and_seller_email_after_commit(order_ids, session_id)

...
class BottomAlignedBlock(Flowable):
    ...
    def __init__(self, flowables, top_padding_mm=0, bottom_padding_mm=6):
        ...
        self._avail_width = 0
        self._avail_height = 0

    def wrap(self, availWidth, availHeight):
        self._avail_width = availWidth
        self._avail_height = availHeight
        self._child_sizes = []
        total_h = 0
...
class _NumberedCanvasImpl:
    def __init__(self, canvas_class, *args, **kwargs):
        self._canvas = canvas_class(*args, **kwargs)
        self._saved_page_states = []
    ...
    def draw_page_number(self, total_pages):
        page_width, _ = A4
        self._canvas.setFont("Roboto", 8)
        self._canvas.setFillColor(colors.black)
        self._canvas.drawRightString(
            page_width - 20 * mm,
            10 * mm,
            f"Page {self._canvas.getPageNumber()} of {total_pages}"
        )
...
def build_bottom_footer_block(data, styles):
    ...
    # Небольшой внутренний верхний отступ, чтобы текст не лип к totals,
    # и при этом если места много — блок опустится вниз страницы.
...
def build_styles():
    ...
    return {
        ...
        # Новые акцентные стили для верхнего блока
...
def draw_later_pages(canvas, doc):
    # На последующих страницах ничего сверху не рисуем.
    # Номер страницы дорисует NumberedCanvas в save().
    pass
...
def build_products_table(data, styles, page_width):
    table_data = [[
        ...
        Paragraph('Total', styles["header_cell"]),
    ]]
    ...
    # Сужаем Unit Price и Total, Description расширяем вправо,
    # при этом общая ширина таблицы остается той же самой.
    col_widths = [
        12 * mm,  # No.
        11 * mm,  # Qty
        24 * mm,  # SKU
        content_width - (12 * mm + 11 * mm + 24 * mm + 26 * mm + 26 * mm),  # Description
        26 * mm,  # Unit Price
        26 * mm,  # Total
    ]
...
def build_totals_table(data, styles, page_width):
    ...
    # Блок totals остается в правой части страницы,
    # но labels получают достаточно ширины и выравниваются влево.
    left_spacer = 95 * mm
...
def build_story(data, styles, page_width):
    ...
    # Чуть меньший отступ под шапку первой страницы
    ...
    # Totals всегда сразу под таблицей
    story.append(build_totals_table(data, styles, page_width))
    story.append(Spacer(1, 5 * mm))

    # А нижний текстовый блок прижимаем вниз отдельно

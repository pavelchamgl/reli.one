...
def get_warehouse_orders_stats(warehouse, days=15):
    ...
    shipment_qty = qs.filter(status=ProductStatus.AWAITING_SHIPMENT).aggregate(Sum('quantity'))['quantity__sum'] or 0
    ...
    awaiting_assembly_and_shipment_qty = awaiting_assembly_qty + shipment_qty
